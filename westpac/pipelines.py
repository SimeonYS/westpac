...
class WwestpacPipeline:
    ...
    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `westpac`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM westpac WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO westpac (date, title, link, content)"
